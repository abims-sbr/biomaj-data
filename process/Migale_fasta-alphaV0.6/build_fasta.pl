...
B<build_fasta.pl> - Convert sequence format to  another sequence with seqret of EMBOSS. By default this script convert sequence to fasta format.
You can use this script in BioMaJ workflow or in standalone mode.

=head1 VERSION

Version 0.6 (January 2008)

=head1 USAGE

=head2 Convert Genbank file to NCBI fasta format

=head3 BioMaJ session

S<build_fasta.pl --biomaj --informat genbank --outinput genbank.fasta=gb*.seq,gb_bct.fasta=gbbct*.seq>

This script create 'genbank.fasta' file with all files matched by 'gb*.seq'.

=head3 Standalone session
...
Create 'genbank.fasta' file in /db/genbank/fasta/ with all files matched by 'gb*.seq' in /db/genbank/flat/.

=head2 Convert NCBI format to NCBI format

S<build_fasta.pl --biomaj --informat ncbi --outinput nr.fasta=nr>

This script create a symbolic link of 'nr' file to 'nr.fasta'

=head2 Same case with several files

S<build_fasta.pl --biomaj --informat ncbi --outinput *.fasta=NC_*.ffn>

For each file matched by 'NC_*.ffn' the script create a symbolic link to '*.fasta'. Joker '*' replace input file name without extension.

=head1 DESCRIPTION

=over

=item - Convert all file format to NCBI format by default

=item - Support convertion one file to one file or N files to one file
