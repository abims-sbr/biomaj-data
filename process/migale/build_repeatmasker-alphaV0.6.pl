...
#parse la ligne de commande outinput et cree un hash
 ...
 ARGUMENTS:{
     ...
     @t_job_args = ( @t_job_args, '-pa', $h_job_args{parallel} )                     if( defined $h_job_args{parallel} );
...
B<build_gcg.pl> - Mask repetitions and low complexity region with RepeatMasker in BioMaJ workflow or in standalone mode.
...
=head2 Mask repetitions in Human genome.
...
=item - Support all RepeatMasker options.

=item - Support batch treatment with joker '*' in input file.
