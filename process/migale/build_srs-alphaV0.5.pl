...
B<build_srs.pl> - SRS indexation in BioMaJ workflow or un standalone mode.
...
=head2 SRS indexation for Genbank
...
=item Support more than one session of SRS indexation with a lock system which create queue.

=item Complete indexation with links.
