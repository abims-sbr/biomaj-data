...
#formatdb qualifiers
	    ...
	    'o'             =>sub{ $h_job_args{seqid_parse} = 'T' },
...
MAIN: {
    ...
    &make_PNal( $h_job_args{sequence_type}, $h_job_args{databank}, $h_job_args{divisions} ) if( defined $h_job_args{databank} );
...
Optional qualifiers:
  ...
  -databank   Name of the databank for *.nal or *.pal. [string] Optional
              Overwrite dbname in BioMaJ session.
	      Without this option Standalone session don t create *.nal or *.pal
...
B<build_blast.pl> - BLAST indexation in BioMaJ workflow or in standalone mode.
...
Create BLAST indexation 'nr' with the file 'nr.fasta'.

=head3 Session Standalone

C<build_blast.pl --outdir /db/nr/blast/ --indir /db/nr/fasta/ --outinput nr=nr.fasta -p T>

Create BLAST indexation 'nr' in directory '/db/nr/blast/' with the file 'nr.fasta' in '/db/nr/fasta/'.

=head2 Genome indexation

=head3 BioMaJ session

C<build_blast.pl --biomaj --outinput *=*.fasta -o T>

Create BLAST indexation for each files selected by '*.fasta' with the same name except '.fasta' extension.
Create an alias file with the name of the databank : genome.nal. You can change this with --databank parameter.
...
=item - Support all formatdb options

=item - Create *.nal and *.pal files if there are most than one file in outinput parameter
